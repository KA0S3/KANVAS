-- Migration: Update load_project and save_project RPC functions to handle cover columns
-- This updates the existing RPC functions to include cover-related fields

-- Drop existing functions first (required to change return type)
DROP FUNCTION IF EXISTS load_project(UUID);
DROP FUNCTION IF EXISTS save_project(UUID, TEXT, TEXT, JSONB, JSONB, JSONB, INT);

-- Recreate load_project to return cover columns
CREATE FUNCTION load_project(
  p_project_id UUID
)
RETURNS TABLE (
  id UUID,
...
-- Recreate save_project to handle cover columns
