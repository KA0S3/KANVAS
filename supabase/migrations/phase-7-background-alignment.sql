-- =====================================================
-- PHASE 7: BACKGROUND SYSTEM ALIGNMENT
-- =====================================================
-- KEEP FRONTEND AS IS - Backend-only changes
-- 
-- GOAL: Move background configs to canonical document at world_document.backgrounds
-- DECISION: Option A - Store in world_document.backgrounds (single source of truth)
--
-- CHANGES:
-- 1. Add UPDATE_GLOBAL_BACKGROUNDS operation for all background configs
-- 2. Add UPDATE_ASSET_BACKGROUND operation for per-asset background (future-proofing)
-- 3. Update save_document_operations function with new handlers
-- =====================================================

-- =====================================================
-- 1. UPDATE SAVE_DOCUMENT_OPERATIONS
-- =====================================================
-- Add new background operations to the main mutation function
-- UPDATE_GLOBAL_BACKGROUNDS: Store all background configs in world_document.backgrounds
-- UPDATE_ASSET_BACKGROUND: Store per-asset background config (alternative/future-proofing)
-- =====================================================
...
UPDATE_CUSTOM_FIELDS, UPDATE_COVER_CONFIG. Returns {success, new_version, error}.';

-- =====================================================
-- 2. BACKGROUND LOADING FUNCTION
-- =====================================================
-- Helper function to load background configs from world_document
-- Used by frontend to restore background state on document load
-- =====================================================

CREATE OR REPLACE FUNCTION public.load_backgrounds(
    p_project_id UUID
)
RETURNS TABLE (
    backgrounds JSONB
...
BEGIN
    RETURN QUERY
    SELECT 
        COALESCE(p.world_document->'backgrounds', '{}'::jsonb) as backgrounds
...
COMMENT ON FUNCTION public.load_backgrounds(UUID) IS 
'Load background configs from world_document.backgrounds. Returns the backgrounds object or empty JSONB if none exists.';
...
SELECT 'PHASE 7 FUNCTION UPDATE' as check_type, 
...
-- PHASE 7 COMPLETE
-- =====================================================
-- Background System now supports:
-- - UPDATE_GLOBAL_BACKGROUNDS: Single source of truth for all background configs
-- - UPDATE_ASSET_BACKGROUND: Per-asset background config (future flexibility)
-- - Backgrounds stored in world_document.backgrounds (consistent with asset storage)
-- - Easier backup/restore of entire book with backgrounds included
--
-- Next step: Update frontend BackgroundStore to use UPDATE_GLOBAL_BACKGROUNDS
-- File: src/services/DocumentMutationService.ts and src/stores/backgroundStore.ts
-- =====================================================
